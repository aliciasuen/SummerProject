...
if(group1){
  ...
  path_to_figure = "/rds/general/project/hda_students_data/live/Group2/Alicia/Summer_project/Figures/Multivariate_obj1/sgPLS/group1/"
  path_to_multi_results = "/rds/general/project/hda_students_data/live/Group2/Alicia/Summer_project/Results/Multivariate_obj1/sgPLS/group1/"
  ## sel prop ##
...
clus_NO2_sp = clus_NO2_sp[clus_NO2_sp>0.00]
pdf(paste(path_to_figure, "sgpls_sp_NO2.pdf", sep =""), width = 5.5, height = 5.5)
par(mar=c(15,5,1,1))
plot(clus_NO2_sp, type="h", lwd=3, las=1, cex.lab=1, bty="L", ylim=c(0,1),
...
for (i in 1:length(clus_NO2_sp)){
  axis(side=1, at=i, labels=names(clus_NO2_sp)[i], las=2, cex.axis=0.6,
...
pdf(paste(path_to_figure, "sgpls_sp_PM10.pdf", sep =""), width = 5.5, height = 5.5)
par(mar=c(15,5,1,1))
